...
# Reuse our existing src modules
...
def reconstruct_sentence(model, tokenizer, sentence, prompt_len):
    ...
    if prompt_len >= len(full_ids): return sentence, 0.0
...
def main():
    config = {
        ...
        'num_repetitions': 1 # Assuming 1 for speed, can be increased
    }
    ...
    validation_sentences = validation_sentences[:100] # For quick testing
    ...
    for file_name in pruned_files:
        ...
        tokenizer = AutoTokenizer.from_pretrained(f"./finetuned_models/{config['base_model_id'].replace('/', '_')}")
        model = AutoModelForCausalLM.from_pretrained(f"./finetuned_models/{config['base_model_id'].replace('/', '_')}")
        ...
        for sentence in tqdm(validation_sentences, desc=f"Testing {model_name}"):
            for theta in config['theta_budgets']:
                latencies = []
                # Simple loop for repetitions
                for _ in range(config['num_repetitions']):
                    recon_text, latency = reconstruct_sentence(model, tokenizer, sentence, theta)
