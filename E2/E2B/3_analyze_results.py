...
def main():
    ...
    if len(N_values) < 3:
        ...
        try:
            ...
            # Plot points from the fit
            plt.scatter(N_values, Q_values, label='Data used for fit', color='red', zorder=5)
            ...
            if not excluded_point.empty:
                plt.scatter(excluded_point['Storage Cost (GB)'].iloc[0] * 1e9, excluded_point['Expected Fidelity'].iloc[0], 
                            label=f'Excluded: {model_to_exclude}', color='gray', marker='x', s=100)
            # Plot the fitted curve
            N_fine = np.linspace(min(N_values), max(N_values), 100)
            plt.plot(N_fine, capacity_scaling_law(N_fine, *popt), label=f'Illustrative Fit (γ={popt[2]:.3f})', color='blue')
            ...
            plot_path = os.path.join(f"./results/{config['experiment_name']}", "illustrative_scaling_law_fit.png")
