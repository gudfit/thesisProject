...
# Assume src/data_handler.py and src/metrics.py exist and are correct
...
def reconstruct_sentence(model, tokenizer, sentence: str, prompt_len: int, device: str):
    ...
    """
    # The 'device' argument is passed in but we robustly get the model's actual device
    # This handles cases where parts of a large model might be on different devices.
    target_device = model.device

    inputs = tokenizer(sentence, return_tensors="pt")
    full_ids = inputs.input_ids[0].to(target_device) # Move the full tensor to the device
    
    # This comparison is now guaranteed to be between two integers.
    if prompt_len >= len(full_ids): 
        return sentence, 0.0
    
    prompt_ids = full_ids[:prompt_len].unsqueeze(0) # No need for .to() again
    ...
    with torch.no_grad():
        output_ids = model.generate(
            ...
            max_new_tokens=len(full_ids) - prompt_len + 5, # Add a buffer for safety
            num_beams=1,
        ...
        for sentence in tqdm(validation_sentences, desc=f"Testing {model_name}"):
            for theta in config['theta_budgets']:
                ...
                for _ in range(config['num_repetitions']):
                    # The call signature is: (model, tokenizer, sentence, integer, string)
                    recon_text, latency = reconstruct_sentence(model, tokenizer, sentence, theta, device)
                ...
                all_results.append({
                    'model_name': model_name, 'storage_cost_lambda': storage_cost,
                    'prompt_len_theta': theta, 'retrieval_cost_ms': avg_latency,
                    'original_sentence': sentence, 'reconstructed_sentence': recon_text,
                    'is_perfect': is_perfect, 'semantic_similarity': sem_sim,
