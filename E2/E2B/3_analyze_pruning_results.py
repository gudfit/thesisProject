# E2/E2B/3_analyze_pruning_results.py
import pandas as pd
import numpy as np
...
def main():
    ...
    summary_df = pd.DataFrame(summary_data).sort_values(
        by="Non-Zero Params (M)", ascending=False
    )
    ...
    eir_data["EIR_scaled"] = (
        ...
        / (eir_data["Non-Zero Params (M)"] * 1e6)
        * EIR_SCALE_FACTOR
    )
    ...
    if len(N_values) < 3:
        ...
        try:
            ...
            print(
                f"  Capacity Scaling Law (γ) Fit Successful! γ = {popt[2]:.4f}, Q_max = {popt[0]:.4f}"
            )
            ...
            plt.scatter(
                N_values,
                Q_values,
                label="Empirical Data (Pruned GPT2-Medium)",
                color="red",
                zorder=5,
            )
            ...
            plt.plot(
                ...
                label=f"Fitted Law (γ={popt[2]:.3f})",
                color="blue",
            )
