#!/usr/bin/env python3
# scripts/compare_compression_methods1.py
"""
...
class ScientificallyValidAnalyzer:
        ...
        for results_dir in self.results_dirs:
            ...
            if 'effective_param_bytes' not in df.columns:
                if 'nonzero_params' in df.columns:
                    df['effective_param_bytes'] = df['nonzero_params'] * 2  # assume fp16 fallback
                else:
                    df['effective_param_bytes'] = df['storage_cost_bytes']
            if 'compression_method' in df and df['compression_method'].iloc[0] == 'basic_pruning':
                df['compression_level'] = df['model_name'].apply(self._extract_pruning_level)
            elif 'compression_method' in df and df['compression_method'].iloc[0] == 'advanced_pruning':
                df['compression_level'] = df['model_name'].apply(self._extract_pruning_level)
            elif 'compression_method' in df and df['compression_method'].iloc[0] == 'quantization':
    ...
    def _analyze_single_architecture_method(self, arch: str, method: str, df: pd.DataFrame):
        ...
        if method in ['basic_pruning', 'advanced_pruning', 'quantization']:
            ...
            for level, group in df_max_theta.groupby('compression_level'):
                storage_gb = group['storage_cost_bytes'].iloc[0] / 1e9 if 'storage_cost_bytes' in group.columns else np.nan
                eff_gb = group['effective_param_bytes'].iloc[0] / 1e9 if 'effective_param_bytes' in group.columns else storage_gb
                if 'nonzero_params' in group.columns:
                    nonzero_m = group['nonzero_params'].iloc[0] / 1e6
                else:
                    if method in ['basic_pruning', 'advanced_pruning'] and isinstance(level, float):
                        base_params = eff_gb * 1e9 / 2 if eff_gb == eff_gb else 0
            ...
            summary_data = []
            success_rate = df_max_theta['is_semantically_equivalent'].mean()
            eff_gb = df_max_theta['effective_param_bytes'].iloc[0] / 1e9 if 'effective_param_bytes' in df_max_theta.columns else (df_max_theta['storage_cost_bytes'].iloc[0] / 1e9 if 'storage_cost_bytes' in df_max_theta.columns else np.nan)
            summary_data.append({
                'Config': f"{arch} ({method})",
                'Storage (GB)': df_max_theta['storage_cost_bytes'].iloc[0] / 1e9 if 'storage_cost_bytes' in df_max_theta.columns else np.nan,
                'Effective (GB)': eff_gb,
                'Non-Zero Params (M)': (df_max_theta['nonzero_params'].iloc[0] / 1e6) if 'nonzero_params' in df_max_theta.columns else np.nan,
                'Success Rate': success_rate,
                'Expected Fidelity': df_max_theta['semantic_similarity'].mean(),
                'Avg Latency (ms)': df_max_theta['retrieval_cost_ms'].mean()
            })
            summary_df = pd.DataFrame(summary_data)
            print(summary_df.to_string(index=False, float_format="%.4f"))
            if len(summary_df) > 0:
    ...
    def _calculate_eir(self, summary_df: pd.DataFrame, full_df: pd.DataFrame):
        ...
        den = summary_df['Effective (GB)'].values * 1e9
        eir = (bzip2_size * summary_df['Expected Fidelity']) / den * EIR_SCALE_FACTOR
        out = summary_df.copy()
        out['EIR_scaled'] = eir
        print(out[['Config', 'Expected Fidelity', 'EIR_scaled']].to_string(index=False, float_format="%.4f"))
    ...
    def _fit_architecture_specific_scaling(self, arch: str, method: str, summary_df: pd.DataFrame):
        ...
        if 'Non-Zero Params (M)' in summary_df.columns and summary_df['Non-Zero Params (M)'].notna().all():
            N_values = summary_df['Non-Zero Params (M)'].values * 1e6
        else:
            N_values = summary_df['Effective (GB)'].values * 1e9
        Q_values = summary_df['Expected Fidelity'].values
        if len(N_values) >= 4 and np.std(N_values) > 0:
            try:
                popt, _ = curve_fit(
                ...
                plt.plot(N_fine, capacity_scaling_law(N_fine, *popt), label=f'Fit (γ={popt[2]:.3f})', color='blue', linewidth=2)
                plt.xlabel('Non-Zero Parameters' if 'Non-Zero Params (M)' in summary_df.columns else 'Effective (Bytes)')
    ...
    def create_method_comparison_plots(self):
        ...
        for arch in architectures:
            ...
            for method in methods:
                ...
                ax1.scatter(grouped['effective_param_bytes'] / 1e9, grouped['semantic_similarity'], label=method, color=color, s=100, alpha=0.7)
            ax1.set_xlabel('Effective (GB)')
            ax1.set_ylabel('Semantic Similarity')
            ax1.set_title('Performance vs Effective Capacity')
            ax1.legend()
            ...
            for method in methods:
                ...
                color = METHOD_COLORS.get(method, '#000000')
                ax2.plot(speed_by_theta.index, speed_by_theta.values, label=method, color=color, marker='o', linewidth=2)
            ...
            for method in methods:
                ...
                color = METHOD_COLORS.get(method, '#000000')
                ax3.plot(success_by_theta.index, success_by_theta.values, label=method, color=color, marker='s', linewidth=2)
            ...
            width = 0.25
            ax4.bar([i - width for i in x], method_summary_df['Avg Similarity'], width, label='Similarity', alpha=0.8)
            ax4.bar(x, method_summary_df['Avg Success'], width, label='Success Rate', alpha=0.8)
            ax4.bar([i + width for i in x], method_summary_df['Avg Speed'] / 1000, width, label='Speed (s)', alpha=0.8)
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            for arch in self.combined_df['base_architecture'].unique():
                ...
                f.write(f"{arch}:\n")
                f.write(f"  Best Similarity: {best_sim['model_name']} ({best_sim['compression_method']}) - {best_sim['semantic_similarity']:.4f}\n")
                arch_df = arch_df.assign(efficiency=arch_df['semantic_similarity'] / arch_df['effective_param_bytes'])
                best_eff = arch_df.loc[arch_df['efficiency'].idxmax()]
                f.write(f"  Most Efficient: {best_eff['model_name']} ({best_eff['compression_method']}) - {best_eff['efficiency']:.6e} sim/byte\n\n")
            f.write("\n" + "="*80 + "\n")
            f.write("Note: Scaling laws are fitted using effective capacity.\n")
...
def main():
    parser = argparse.ArgumentParser(description="Scientifically valid analysis of compression methods")
    parser.add_argument('--results-dirs', nargs='+', required=True, help='Directories containing experiment results')
    parser.add_argument('--output-dir', type=str, default='results/scientific_analysis', help='Output directory for analysis results')
