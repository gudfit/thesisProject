# E1/src/data/factual_probe_generator.py
import re
import random
import spacy
...
class FactualProbeGenerator:
    def __init__(self, spacy_model: str = "en_core_web_sm"):
        try:
            ...
            self.nlp = spacy.load(spacy_model)
        self.patterns = {
            'is_a': r'(.+?)\s+(?:is|are|was|were)\s+(?:a|an|the)?\s*(.+)',
            'located_in': r'(.+?)\s+(?:in|at|on|near)\s+(.+)',
            'year_fact': r'(?:In|During|Around)?\s*(\d{4})\s*,?\s*(.+?)(?:\.|,|;)',
            'numerical_fact': r'(.+?)\s+(?:has|have|had|contains?|is|are|was|were)\s+(\d+(?:,\d+)*(?:\.\d+)?)\s+(\w+)',
            'definition': r'(.+?)\s+(?:is defined as|means|refers to)\s+(.+?)(?:\.|,|;)',
        }
    ...
    def _extract_numerical_facts(self, sentence: str) -> List[Dict[str, str]]:
        probes = []
        num_pattern = r'(.+?)\s+(?:has|have|had|contains?|is|are|was|were)\s+(\d+(?:,\d+)*(?:\.\d+)?)\s+(\w+)'
    ...
    def _extract_definition_facts(self, sentence: str) -> List[Dict[str, str]]:
        probes = []
        def_patterns = [
            r'(.+?)\s+is\s+(?:a|an|the)\s+(\w+(?:\s+\w+){0,3})',
            r'(.+?),\s+(?:a|an|the)\s+(\w+(?:\s+\w+){0,3})',
        ]
    ...
    def _create_capital_probe(self, sentence: str, location: str) -> Optional[Dict[str, str]]:
        if "capital of" in sentence.lower():
            match = re.search(r'capital\s+of\s+(.+)', sentence, re.IGNORECASE)
            if match:
