...
from src.models.simple_lossy_compressor import SimpleLossyCompressor
...
class Experiment1CRunner:
    ...
    def run_compression_analysis(self):
        ...
        for fidelity_method in fidelity_methods:
            self.logger.info(f"\n{'#'*70}\n# Running full analysis with Structural Fidelity Method: '{fidelity_method}'\n{'#'*70}")
            results = {}
    ...
    def _run_and_log_baselines(self, test_texts: List[str]):
        self.logger.info("\n" + "="*80)
        self.logger.info("--- Establishing Compressor Baselines ---")

        trad_compressor = TraditionalCompressors()
        trad_results = trad_compressor.benchmark_all_algorithms(test_texts)
        self.logger.info("\n--- Traditional Lossless Compressor Benchmark ---")
        header = f"{'Algorithm':<10} | {'Bits/Char':>10} | {'Comp Speed (MB/s)':>18}"
        self.logger.info(header + "\n" + "-"*len(header))
        for name, metrics in trad_results.items():
            self.logger.info(
                f"{name:<10} | {metrics['avg_bits_per_character']:>10.3f} | {metrics['avg_compression_speed_mbps']:>18.2f}"
            )

        self.logger.info("\n--- Simple Lossy Compressor Benchmark (for Fidelity Comparison) ---")
        simple_compressor = SimpleLossyCompressor()
        crumpled_metrics = CrumpledPaperMetrics()
        
        header = f"{'Damage Level':<15} | {'Bits/Char':>10} | {'Mean TCM':>10} | {'Mean PCM':>10}"
        self.logger.info(header + "\n" + "-"*len(header))
        
        for n in [10, 5, 3]:
            all_tcm, all_pcm, all_bpc = [], [], []
            for text in test_texts:
                if not text: continue
                reconstructed_text = simple_compressor.compress(text, n=n)
                metrics = crumpled_metrics.calculate_crease_metrics(text, reconstructed_text)
                bpc = simple_compressor.get_bits_per_character(text, reconstructed_text)
                all_tcm.append(metrics['total_crease_magnitude'])
                all_pcm.append(metrics['peak_crease_magnitude'])
                all_bpc.append(bpc)
            
            self.logger.info(
                f"{f'1-in-{n} removed':<15} | {np.mean(all_bpc):>10.3f} | {np.mean(all_tcm):>10.2f} | {np.mean(all_pcm):>10.2f}"
            )
        self.logger.info("="*80)
    ...
    def _evaluate_all_variants(self, variants: Dict, model_name: str, compressor: ModelCompressor, fidelity_method: str) -> Dict:
        ...
        for key, info in variants.items():
            ...
            if fidelity_method == 'predictive_masking':
                structural_results = self._evaluate_structural_fidelity_pm(info['model'], model_name, test_texts)
            else: 
                structural_results = self._evaluate_structural_fidelity_direct(info['model'], compressor.tokenizer, test_texts)
    ...
    def _evaluate_structural_fidelity_direct(self, model: torch.nn.Module, tokenizer, test_texts: List[str]) -> Dict:
