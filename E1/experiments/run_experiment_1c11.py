...
class Experiment1CRunner:
    ...
    def run_compression_analysis(self):
        ...
        # Run and log the traditional compressor benchmark first
        self._run_and_log_traditional_benchmark(test_texts)
    ...
    def _run_and_log_traditional_benchmark(self, test_texts: List[str]):
        self.logger.info("\n" + "="*80)
        self.logger.info("--- Establishing Traditional Compressor Baseline ---")
        trad_compressor = TraditionalCompressors()
        benchmark_results = trad_compressor.benchmark_all_algorithms(test_texts)
        self.logger.info("Traditional Compressor Benchmark Results:")
        header = f"{'Algorithm':<10} | {'Compression Ratio':>18} | {'Bits/Char':>10} | {'Comp Speed (MB/s)':>18} | {'Decomp Speed (MB/s)':>20}"
        self.logger.info(header)
        self.logger.info("-" * len(header))
        for name, metrics in benchmark_results.items():
            self.logger.info(
                f"{name:<10} | {metrics['avg_compression_ratio']:>18.2f}x | {metrics['avg_bits_per_character']:>10.3f} | "
                f"{metrics['avg_compression_speed_mbps']:>18.2f} | {metrics['avg_decompression_speed_mbps']:>20.2f}"
            )
        self.logger.info("="*80)
    ...
    def _evaluate_structural_fidelity_pm(self, model: torch.nn.Module, model_name: str, test_texts: List[str]) -> Dict:
        ...
        if test_texts: self._run_gzip_baseline_check(crumpled_metrics, test_texts[0])
    ...
    def _run_gzip_baseline_check(self, crumpled_metrics: CrumpledPaperMetrics, sample_text: str):
        ...
        self.logger.info(f"Lossless Gzip Sanity Check: TCM={gzip_metrics['total_crease_magnitude']:.1f}, PCM={gzip_metrics['peak_crease_magnitude']:.1f} (should be 0.0)")
